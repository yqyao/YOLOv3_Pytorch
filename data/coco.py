...
class COCODetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, resize_wh, batch_size, multiscale=False, dataset_name='COCO'):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        if self.multiscale:
            if index % (self.batch_size * 20) == 0:
                rnd = (random.randint(0,9) + 10) * 32
                print("resize scale", index, rnd)
                self.resize_wh = (rnd, rnd)
        img = cv2.imread(img_id, cv2.IMREAD_COLOR)
        height, width, _ = img.shape

        if self.transform is not None:
            img, target = self.transform(img, target, self.resize_wh)
